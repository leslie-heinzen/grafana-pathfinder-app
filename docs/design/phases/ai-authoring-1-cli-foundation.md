...
The next phase (P2 — npm + Docker distribution) inherits a fully-functional authoring CLI with importable command functions. P3 (TypeScript MCP server) inherits both the importable command surface and the `--help --format json` stability contract.

> **Note (2026-04-29 design pivot).** The original P1 handoff mentioned a bundled single-file binary path. That approach has been retired — the MCP server is now a TypeScript sibling entrypoint that imports CLI commands as library functions, and the npm package is the distribution unit. Updated handoff bullets below reflect the new direction; the remainder of this phase plan stands.

- **CLI is end-to-end functional.** All eight authoring commands plus `inspect` and the existing `validate` work against any package directory. Validate-on-write is enforced for every mutation; invalid state never persists. Verified by 178 new unit + integration tests and a full subprocess smoke run against the built CLI.
...
- **`--help --format json` is frozen.** The shape (`command`, `summary`, `required`, `optional`, `addressing`, `subcommands` plus per-flag `name`, `valueType`, `enum`, `repeatable`, `description`, `default`) is locked by `output.test.ts`. P3's `pathfinder_help` tool composes the same surface in-process — it imports the same code that produces this output, no shell-out required.
- **Distribution path for P2.** The existing `npm run build:cli` produces the dist artifact at `dist/cli/cli/index.js`. P2 wraps the package for npm publish and Docker — no Node SEA, no `pkg`, no plugin-tarball coupling. P3 adds a second `pathfinder-mcp` entrypoint to the same npm package.
- **Reusable test scaffolding.** `src/cli/__tests__/commands.test.ts` and `authoring-integration.test.ts` use exported `runX` functions (not subprocess invocations), keeping the test suite fast (~2s for the full CLI suite). P3's MCP tool dispatchers compose against the same `runX` surface — the MCP layer's tests can extend the existing fixtures without standing up a subprocess harness.
